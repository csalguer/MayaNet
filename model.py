...
class HED(nn.Module):
    ...
    def forward(self, x):
        ...
        ## side output
        d1 = self.dsn1(conv1)
        d2 = F.interpolate(self.dsn2(conv2), size=(h,w))
        d3 = F.interpolate(self.dsn3(conv3), size=(h,w))
        d4 = F.interpolate(self.dsn4(conv4), size=(h,w))
        d5 = F.interpolate(self.dsn5(conv5), size=(h,w))
